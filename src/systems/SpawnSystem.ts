...
import { SpawnerComponent, SpawnTiming, type EnemyType, type SpawnWave } from '../components/Spawner'
...
export class SpawnSystem extends System {
  ...
  initialize(): void {
    if (this.eventSystem) {
      ...
      this.eventSystem.on('BOSS_PHASE_CHANGE', this.handleBossPhaseChange.bind(this))
  ...
  update(context: SystemUpdateContext, entities: EntityQuery[]): void {
    ...
    spawnerEntities.forEach(spawner => {
      if (this.config.performanceMode && this.spawnsThisFrame >= this.maxSpawnsPerFrame) {
        return
      }
    ...
    if (this.config.dynamicDifficulty && currentTime - this.lastUpdateTime > 10000) {
   ...
   */
  private updateSpawner(spawner: SpawnerEntityQuery, currentTime: number, context: SystemUpdateContext): void {
    ...
    if (this.config.difficultyScaling) {
      ...
      spawnerComponent.difficultyMultiplier = this.currentDifficulty.difficultyMultiplier
    }
    ...
    if (this.config.bossSpawnEnabled && spawnerComponent.spawnTiming === SpawnTiming.BOSS) {
   ...
   */
  private processSpawning(spawner: SpawnerEntityQuery, currentTime: number): void {
    const spawnerComponent = spawner.components.spawner

    // Check global limits
    if (this.globalEnemyCount >= this.config.maxGlobalEnemies) return
    if (spawnerComponent.currentActiveEnemies >= this.config.maxEnemiesPerSpawner) return

    // Check spawn budget
    if (this.spawnsThisFrame >= spawnerComponent.spawnBudget) return

    // Select enemy type based on current difficulty
    const enemyType = spawnerComponent.selectEnemyType(this.currentDifficulty.playerLevel)
    if (!enemyType) return
    ...
    // Record spawn
    spawnerComponent.recordSpawn(enemy.id, enemyType.id, spawnPosition, currentTime)
   ...
   */
  private processBossSpawning(spawner: SpawnerEntityQuery, currentTime: number): void {
    const spawnerComponent = spawner.components.spawner

    // Check if boss already exists
    if (spawnerComponent.bossEntityId && this.activeBosses.has(spawnerComponent.bossEntityId)) {
      return
    }
   ...
   */
  private createEnemy(enemyType: EnemyType, position: Vector2, spawnerId: EntityId): EntityQuery | null {
    ...
    const scaledHealth = this.scaleValue(baseHealth, enemyType.scalingFactors?.health || 1)
    const health = new HealthComponent(scaledHealth)
    entity.addComponent(health)

    // Add movement component
    const baseSpeed = 50
    const scaledSpeed = this.scaleValue(baseSpeed, enemyType.scalingFactors?.speed || 1)
    const movement = new MovementComponent(scaledSpeed)
    entity.addComponent(movement)

    // Add combat component
    const baseDamage = 10
    const scaledDamage = this.scaleValue(baseDamage, enemyType.scalingFactors?.damage || 1)
    const combat = new CombatComponent({
      damage: { min: scaledDamage * 0.8, max: scaledDamage * 1.2 },
      attackSpeed: 1.0,
      range: 50,
      criticalChance: 0.1,
      criticalDamage: 1.5
    ...
    enemyType.components.forEach(template => {
      if (['transform', 'health', 'movement', 'combat', 'ai'].includes(template.type)) {
   ...
   */
  private createBoss(bossType: EnemyType, position: Vector2, spawnerId: EntityId): EntityQuery | null {
    if (!this.world) return null
    ...
    const scaledHealth = this.scaleValue(baseHealth, bossType.scalingFactors?.health || 5)
    const health = new HealthComponent(scaledHealth)
    entity.addComponent(health)

    // Add movement component (bosses are slower)
    const baseSpeed = 30
    const scaledSpeed = this.scaleValue(baseSpeed, bossType.scalingFactors?.speed || 0.8)
    const movement = new MovementComponent(scaledSpeed)
    entity.addComponent(movement)

    // Add combat component with boss stats
    const baseDamage = 50
    const scaledDamage = this.scaleValue(baseDamage, bossType.scalingFactors?.damage || 3)
    const combat = new CombatComponent({
      damage: { min: scaledDamage * 0.9, max: scaledDamage * 1.1 },
      attackSpeed: 0.5,
      range: 100,
      criticalChance: 0.25,
      criticalDamage: 2.0,
   ...
   */
  private createComponentFromTemplate(template: any, enemyType: EnemyType): any {
  ...
  private scaleValue(baseValue: number, scalingFactor: number): number {
    return baseValue * scalingFactor * this.currentDifficulty.difficultyMultiplier
  }

  /**
   * Checks if spawn position is valid
   */
  private isValidSpawnPosition(position: Vector2, enemyType: EnemyType): boolean {
    ...
    for (const entity of nearbyEntities) {
      const hasAI = (entity as any).hasComponent('ai')
      if (!hasAI) { // Assume non-AI entities are players
        return false // Too close to player
      }
    }

    // Check density (prevent overcrowding)
    const nearbyEnemies = nearbyEntities.filter((e: any) => e.hasComponent('ai'))
  ...
  private updateDynamicDifficulty(): void {
    ...
    const killRate = this.enemiesKilledThisWave / (this.currentDifficulty.survivalTime / 1000)
    ...
    // Clamp difficulty
    this.currentDifficulty.difficultyMultiplier = Math.max(0.5, Math.min(5.0, this.currentDifficulty.difficultyMultiplier))
  }
    ...
    if (this.world) {
      ...
      if (entity?.hasComponent('ai')) {
        ...
        this.activeSpawners.forEach(spawnerId => {
          ...
          if (spawner) {
            const spawnerComponent = spawner.getComponent('spawner') as SpawnerComponent
   ...
   */
  private spawnBossAdds(bossId: EntityId, bossPosition: Vector2, phase: number): void {
   ...
   */
  createSpawner(position: Vector2, config: Partial<SpawnerComponent>): EntityQuery | null {
    if (!this.world) return null
