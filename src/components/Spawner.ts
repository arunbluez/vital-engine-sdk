...
export class SpawnerComponent extends Component {
   ...
   */
  tryStartNextWave(currentTime: number, gameState: Record<string, unknown>): boolean {
    if (this.currentWave) {
  ...
  private checkWaveRequirement(
    ...
    switch (requirement.type) {
      ...
      case 'enemy_count':
        return this.compareValues(this.currentActiveEnemies, requirement.operator, requirement.value as number)
      
      case 'boss_defeated':
        return (gameState.bossesDefeated as string[] | undefined)?.includes(requirement.value as string) ?? false
  ...
  selectEnemyType(currentLevel: number): EnemyType | null {
    const validTypes = this.enemyTypes.filter(type => 
      currentLevel >= type.minLevel && currentLevel <= type.maxLevel
    )
   ...
   */
  recordSpawn(entityId: EntityId, enemyType: string, position: Vector2, currentTime: number): void {
  ...
  recordEnemyDeath(entityId: EntityId): void {
    ...
    const historyIndex = this.spawnHistory.findIndex(event => event.entityId === entityId)
    if (historyIndex !== -1) {
   ...
   */
  updateSpawnRates(gameTime: number, playerLevel: number, survivalTime: number): void {
    // Time-based scaling
    ...
    const totalMultiplier = this.timeMultiplier * this.levelMultiplier * this.survivalMultiplier * this.difficultyMultiplier
  ...
  clone(): SpawnerComponent {
    const clone = new SpawnerComponent({ ...this.spawnArea, center: { ...this.spawnArea.center } })
    ...
    clone.enemyTypes = this.enemyTypes.map(type => ({
      ...type,
      components: type.components.map(comp => ({ ...comp, data: { ...comp.data } })),
      scalingFactors: type.scalingFactors ? { ...type.scalingFactors } : undefined
    }))
