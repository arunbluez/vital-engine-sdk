...
import { DifficultyComponent, DifficultyLevel, ScalingMetric, DifficultyModifier } from '../components/Difficulty'
import { HealthComponent } from '../components/Health'
import { CombatComponent } from '../components/Combat'
import { MovementComponent } from '../components/Movement'
import { SpawnerComponent } from '../components/Spawner'
import { EnemyAIComponent } from '../components/EnemyAI'
import { DifficultyChangedEvent } from '../types/Events'
...
export class DifficultySystem extends System {
  ...
  public update(context: SystemUpdateContext): void {
    ...
    if (currentTime - this.lastDifficultyUpdate < this.difficultyUpdateInterval) {
      return
    }
    
    const difficultyEntities = this.world.queryEntitiesWithComponents(['difficulty']) as DifficultyEntityQuery[]
  ...
  private updateDifficultyMetrics(entity: DifficultyEntityQuery, deltaTime: number): void {
  ...
  private processAdaptiveDifficulty(entity: DifficultyEntityQuery, currentTime: number): void {
    const difficulty = entity.difficulty
    
    if (!difficulty.adaptiveSettings.isEnabled) return
    
    const timeSinceLastAdaptation = currentTime - difficulty.lastAdaptationTime
    if (timeSinceLastAdaptation < difficulty.adaptiveSettings.adaptationInterval) return
    
    const performanceScore = difficulty.getPerformanceScore()
    difficulty.performanceHistory.push(performanceScore)
    
    if (difficulty.performanceHistory.length > difficulty.adaptiveSettings.performanceWindowSize) {
      difficulty.performanceHistory.shift()
    }
    
    if (difficulty.performanceHistory.length < 3) return
    
    const averagePerformance = difficulty.performanceHistory.reduce((a, b) => a + b, 0) / difficulty.performanceHistory.length
    const targetRange = difficulty.adaptiveSettings.targetPerformanceRange
    
    let adjustment = 0
    
    if (averagePerformance < targetRange.min) {
      adjustment = (targetRange.min - averagePerformance) * difficulty.adaptiveSettings.adaptationRate * -1
    } else if (averagePerformance > targetRange.max) {
      adjustment = (averagePerformance - targetRange.max) * difficulty.adaptiveSettings.adaptationRate
    }
    
    if (Math.abs(averagePerformance - ((targetRange.min + targetRange.max) / 2)) > difficulty.adaptiveSettings.emergencyAdjustmentThreshold) {
      adjustment *= 2
    }
    
    adjustment = Math.max(-difficulty.adaptiveSettings.maxAdjustmentPerInterval, 
                        Math.min(difficulty.adaptiveSettings.maxAdjustmentPerInterval, adjustment))
    
    if (Math.abs(adjustment) > 0.01) {
      const newTargetScore = Math.max(0, difficulty.targetScore + (adjustment * 100))
      
      const transitionResult = difficulty.shouldTransitionDifficulty(newTargetScore)
    ...
    const performanceVariance = this.calculateVariance(difficulty.performanceHistory)
    difficulty.isStabilized = performanceVariance < difficulty.adaptiveSettings.stabilityThreshold
  }
  ...
  private changeDifficultyLevel(entity: DifficultyEntityQuery, newLevel: DifficultyLevel): void {
    const difficulty = entity.difficulty
    ...
    const newBand = difficulty.getDifficultyBandForScore(difficulty.currentScore)
    if (newBand) {
    ...
    this.world.eventSystem.emit('difficulty_changed', difficultyEvent)
  ...
  private applyDifficultyModifiers(entity: DifficultyEntityQuery): void {
    const difficulty = entity.difficulty
    const scalableEntities = this.world.queryEntitiesWithComponents([]) as ScalableEntityQuery[]
    
    for (const [modifierId, modifier] of difficulty.activeModifiers) {
      const appliedEntities = this.getEntitiesForModifier(scalableEntities, modifier)
      
      for (const targetEntity of appliedEntities) {
        this.applyModifierToEntity(targetEntity, modifier, difficulty.currentScore)
      }
    }
  }
  
  private getEntitiesForModifier(entities: ScalableEntityQuery[], modifier: DifficultyModifier): ScalableEntityQuery[] {
  ...
  private applyModifierToEntity(entity: ScalableEntityQuery, modifier: DifficultyModifier, difficultyScore: number): void {
    const targetProperty = modifier.targetProperty
    ...
    if (targetProperty === 'health.maxHealth' && entity.health) {
      ...
      const scaledHealth = modifier.calculateModifierValue(modifier, normalizedScore) * baseHealth
      ...
      const scaledDamage = modifier.calculateModifierValue(modifier, normalizedScore) * baseDamage
      ...
      const scaledSpeed = modifier.calculateModifierValue(modifier, normalizedScore) * baseSpeed
      ...
      const baseRate = entity.spawner.baseSpawnRate || entity.spawner.currentSpawnRate
      const scaledRate = modifier.calculateModifierValue(modifier, normalizedScore) * baseRate
      ...
      const baseVariety = entity.spawner.baseEnemyVariety || entity.spawner.maxConcurrentEnemyTypes
      const scaledVariety = modifier.calculateModifierValue(modifier, normalizedScore) * baseVariety
      entity.spawner.maxConcurrentEnemyTypes = Math.max(1, Math.round(scaledVariety))
      if (!entity.spawner.baseEnemyVariety) entity.spawner.baseEnemyVariety = baseVariety
    }
    
    else if (targetProperty === 'spawner.bossFrequency' && entity.spawner) {
      const baseBossRate = entity.spawner.baseBossSpawnRate || 0.1
      const scaledBossRate = modifier.calculateModifierValue(modifier, normalizedScore) * baseBossRate
      entity.spawner.bossSpawnRate = Math.min(1.0, scaledBossRate)
      if (!entity.spawner.baseBossSpawnRate) entity.spawner.baseBossSpawnRate = baseBossRate
    }
    
    else if (targetProperty === 'enemyAI.aggressionLevel' && entity.enemyAI) {
      const baseAggression = entity.enemyAI.baseAggressionLevel || 1.0
      const scaledAggression = modifier.calculateModifierValue(modifier, normalizedScore) * baseAggression
      entity.enemyAI.aggressionLevel = Math.max(0.1, Math.min(3.0, scaledAggression))
      if (!entity.enemyAI.baseAggressionLevel) entity.enemyAI.baseAggressionLevel = baseAggression
    }
    
    else if (targetProperty === 'all') {
      if (entity.health) this.applyModifierToEntity(entity, { ...modifier, targetProperty: 'health.maxHealth' }, difficultyScore)
      if (entity.combat) this.applyModifierToEntity(entity, { ...modifier, targetProperty: 'combat.damage' }, difficultyScore)
      if (entity.movement) this.applyModifierToEntity(entity, { ...modifier, targetProperty: 'movement.speed' }, difficultyScore)
      if (entity.spawner) {
        this.applyModifierToEntity(entity, { ...modifier, targetProperty: 'spawner.spawnRate' }, difficultyScore)
        this.applyModifierToEntity(entity, { ...modifier, targetProperty: 'spawner.bossFrequency' }, difficultyScore)
      }
      if (entity.enemyAI) this.applyModifierToEntity(entity, { ...modifier, targetProperty: 'enemyAI.aggressionLevel' }, difficultyScore)
    }
  }
  ...
  public setDifficultyLevel(entityId: EntityId, level: DifficultyLevel): boolean {
    const entity = this.world.getEntity(entityId)
    ...
    const targetBand = difficulty.difficultyBands.find(band => band.level === level)
  ...
  public enableAdaptiveDifficulty(entityId: EntityId, enabled: boolean): boolean {
    const entity = this.world.getEntity(entityId)
    if (!entity) return false
  ...
  public addCustomModifier(
    ...
    if (targetLevel) {
      const targetBand = difficulty.difficultyBands.find(band => band.level === targetLevel)
      if (targetBand) {
