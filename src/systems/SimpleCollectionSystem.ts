...
export class SimpleCollectionSystem extends System {
  ...
  public update(context: SystemUpdateContext, entities: EntityQuery[]): void {
    ...
    if (this.world?.getActiveEntities) {
      allEntities = this.world.getActiveEntities() as Entity[]

      // Find collectors (entities with magnet)
      collectors = allEntities.filter(
        (e: Entity) =>
          e.hasComponent &&
          e.hasComponent('magnet') &&
          e.hasComponent('transform')
      )

      // Find collectibles
      collectibles = allEntities.filter(
        (e: Entity) =>
          e.hasComponent &&
  ...
  private processCollectionPair(
    collector: Entity,
    collectible: Entity,
    deltaTime: number
  ): void {
    const collectorTransform = collector.getComponent(
      'transform'
    ) as TransformComponent
    const collectorMagnet = collector.getComponent('magnet') as MagnetComponent
    const collectibleTransform = collectible.getComponent(
      'transform'
    ) as TransformComponent
    const collectibleComp = collectible.getComponent(
      'collectible'
    ) as CollectibleComponent
